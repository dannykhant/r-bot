...
def can_be_optimized_by_index_min_max(sql_query: str, schema: str = None, indexes: str = None, dialect: str = None) -> bool:
    """
    Check if the given SQL query can be optimized for MIN/MAX by using index.
    """
    ...
    # Check if MIN, MAX can benefit from any index with the help of equality conditions on the index columns
    for node in parsed.find_all(exp.Min, exp.Max):
        for index_columns in parsed_indexes.values():
            for col in index_columns:
                if node.this.this == col:
                    return True
                if col not in equal_columns:
                    break
